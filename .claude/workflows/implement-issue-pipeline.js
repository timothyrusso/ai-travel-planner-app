...
1. CONVERT — in a scratch directory, for every image of the plan, FIRST prove the source really is a PNG that THIS repository's QA wrote for this issue. Define this function once and run it per source:
   ...
   Drop — and note in \`note\` — every image that fails, and never make it fatal: the whole visual summary is best-effort and cannot change the run verdict. Be clear-eyed about what this buys: these bytes go to a PUBLIC branch and the pipeline's own path check is lexical (it has no filesystem), so this is the only place the real file is inspected — but it is a blast-radius reducer, not a sandbox. It stops malformed paths, links out of the evidence directory, hard links to plaintext secrets and bytes that do not even open with the PNG magic number — but a header is not an image, so it is the conversion below, not this check, that decides whether the rest of the file is really a picture; it cannot stop a genuinely hostile agent, which runs as the same user and could encode any readable file into a valid PNG. Then convert what survived:
   \`ffmpeg -y -i "<source>" -vf "scale='if(gt(iw,ih),min(1080,iw),-2)':'if(gt(iw,ih),-2,min(1080,ih))'" "<scratch>/<name>.webp"\`
   (caps the long edge at 1080 px, whichever edge that is). Then branch on ffmpeg's EXIT CODE — never on a single "it did not work" catch-all, because the three outcomes need opposite handling:
   - ffmpeg ABSENT — exit code 127, command not found. Nothing ever looked at the bytes, so a genuine screenshot must still reach the PR: copy the source unchanged to \`<scratch>/<name>.png\`. This is the ONLY case in which the copy-unchanged fallback is allowed.
   - ffmpeg RAN and REJECTED the input — any other non-zero exit code (it answers 69 on data it cannot decode), or exit 0 with an empty output file. ffmpeg is a decoder: its refusal is itself proof that the source is not a decodable image, whatever its first 8 bytes claim. DROP that image, and \`rm -f\` the output it left behind — a failed run writes a 0-byte \`.webp\`, and step 2's \`find\` would otherwise sweep that corpse onto the public branch. Record the drop in \`note\`. Do NOT fall back to copying the source: bytes ffmpeg just refused to decode are exactly the bytes that must not be published — a screenshot truncated by a crashed capture, or a file that is nothing but a PNG header glued in front of something else.
   - ffmpeg SUCCEEDED — exit code 0 and a non-empty \`.webp\`. Publish that.
   Remember each file's ACTUAL extension: the markdown must link the file you really produced.
