...
/**
 * Per-size geometry of the tag, component-scoped on purpose: the letter-spacings 0.72/0.8/0.96 are
 * one-off values only this tag asks for, and adding them to a shared ladder would offer every other
 * component a step it must never use. Unlike the chip's, the radius is flat at every size — a tag is
 * a rounded rectangle, never a pill — and the heights are the chip's tokens because the two
 * components share one height scale, so one set of tokens is correct even read from here.
 */
